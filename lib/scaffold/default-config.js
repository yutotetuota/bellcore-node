 ...
 * configuration file in the "~/.bellcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of Bellcore.`);

    var now = new Date();
    // bellcore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `bellcore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
  ...
  var defaultConfig = {
    ...
    servicesConfig: {
      'insight-api-bellcoin': {
        cwdRequirePath: 'node_modules/insight-api-bellcoin'
      },
      'insight-ui-bellcoin': {
        cwdRequirePath: 'node_modules/insight-ui-bellcoin'
      }
    }
  };
