...
function start(options) {
  ...
  var node = new BellcoreNode(fullConfig);
...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.bellcoreNode) {
      serviceModule = serviceModule + '/' + servicePackage.bellcoreNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a bellcoreNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - bellcoreNode field. All paths failed to find valid nodeJS code.');
  }
