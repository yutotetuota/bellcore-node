...
describe('Address Service', function() {
  ...
  describe('#_getAddressTxSummary', function() {
    it('should get the address txid history', function(done) {
      ...
      addressService._getAddressTxSummary('a', null, function(err, txList) {
        ...
        expect(txIdList).to.deep.equal([{txid: tx.txid(), height: 10, input: 1, satoshis: 213.55}]);
        done();
  ...
  describe('#AddressSummary', function() {
    ...
    it('should get the address summary, incoming', function(done) {
      ...
      addressService.getAddressSummary('BA5wcXSY5ozpa8AGSLGPtendp5d2BzGBWE', {}, function(err, res) {
        if (err) {
          return done(err);
        }
        expect(res).to.deep.equal({ addrStr: 'BA5wcXSY5ozpa8AGSLGPtendp5d2BzGBWE',
          balance: 0.005,
          balanceSat: 500000,
          totalReceived: 0.005,
